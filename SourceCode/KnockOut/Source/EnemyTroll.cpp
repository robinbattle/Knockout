...
namespace KnockOut
{
	...
	void EnemyTroll::setUp()
	{
		
		userGUI = UserGUI::getSingletonPtr();
		entity = mSceneManager->createEntity(appropriateName(ENTITY), "troll.mesh");
		...
		reachHome = false;
		saveBody = 5.0f;
        ps = mSceneManager->createParticleSystem("EnemyTrollBlast" + convertInt(ID), "TRPlayer/Torch");
	...
	std::string EnemyTroll::appropriateName(NAME_TYPE type)
	{
		if(type == SCENE_NODE)
			return "EnemyTrollScenenode" + convertInt(ID);
		else if(type == ENTITY)
			return "EnemyTrollEntity" + convertInt(ID);
	...
	void EnemyTroll::update(float time)
	{
