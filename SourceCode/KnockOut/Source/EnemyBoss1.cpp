...
namespace KnockOut
{
	...
	void EnemyBoss1::setUp()
	{
		
		userGUI = UserGUI::getSingletonPtr();
		entity = mSceneManager->createEntity(appropriateName(ENTITY), "boss1.mesh");
		...
		reachHome = false;
		saveBody = 5.0f;
        ps = mSceneManager->createParticleSystem("EnemyBoss1Blast" + convertInt(ID), "TRPlayer/Torch");
		sceneNode->attachObject(ps); 
		ps->setVisible(false);
		lifeValue = 550;
		worthMoney = 15;
		
		mtext = new MovableText("EnemyBoss1TXT" + convertInt(ID), convertInt(lifeValue), "StarWars");
	...
	std::string EnemyBoss1::appropriateName(NAME_TYPE type)
	{
		if(type == SCENE_NODE)
			return "EnemyBoss1Scenenode" + convertInt(ID);
		else if(type == ENTITY)
			return "EnemyBoss1Entity" + convertInt(ID);
	...
	void EnemyBoss1::update(float time)
	{
